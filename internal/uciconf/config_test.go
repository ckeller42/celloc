...
nodotline=value`
	cfg := uciconf.ParseUciShow(out)
	def := uciconf.Defaults()
	if cfg.Listen != def.Listen || cfg.Bus != def.Bus || cfg.Runner != def.Runner {
		t.Fatalf("empty values clobbered defaults: %+v", cfg)
	}
}

func TestParseUciShowZeroIntervalKeepsDefault(t *testing.T) {
	cfg := uciconf.ParseUciShow(`geolocd.main.wifi_interval='0'`)
	if cfg.WifiInterval != 300*time.Second {
		t.Fatalf("zero interval should keep default, got %v", cfg.WifiInterval)
	}
}

func TestParseUciShowIgnoresBadIntervalAndUnknown(t *testing.T) {
	out := `geolocd.main.wifi_interval='notanumber'
geolocd.main.bogus='x'
unrelated line`
	cfg := uciconf.ParseUciShow(out)
	if cfg.WifiInterval != 300*time.Second {
		t.Fatalf("bad interval should keep default, got %v", cfg.WifiInterval)
	}
}
